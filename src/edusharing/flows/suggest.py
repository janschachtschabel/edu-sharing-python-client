...
async def accept_suggestion(
    ...
    and with the read-back every write in this library has.
    ...
    Returns:
        ...
        ``applied`` is ``True`` only when the value was read back and the
        proposal is now ``ACCEPTED``. Otherwise ``status`` is what it still is
        and ``failed`` names the part that stopped it: ``status`` for a
        proposal already decided, ``apply`` for a value the repository did not
        keep -- then nothing was marked, and the proposal stays open.
    ...
    answer: dict[str, Any] = {
        ...
        "applied": False, "status": match.status, "failed": [],
    }
