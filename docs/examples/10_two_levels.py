...
def main() -> int:
    ...
    with Repository.from_env(metadataset="mds_oeh", client=client) as repo:
        ...
        # ── Use case 1: search with a subject filter ──────────────────────
        ...
        api_subjects = first.labels("ccm:taxonid")
        ...
        # ── Use case 2: publish material into a collection ────────────────
        ...
        try:
            ...
            # Flow level — one call. Same work, and it reports what did not
            # resolve instead of writing material that looks tagged and is not.
            fresh(repo)
            ...
            print(f"  unresolved reported by the flow: "
                  f"{created['unresolved'] or 'none'}")
            ...
            # ── Where the flow genuinely saves a round trip ────────────────
        ...
        for hit in found["hits"]:
            try:
                ...
                # Not a contrived case: the index holds records whose node is
                # gone. Measured 2026-08-27, 4 of 25. Anything chaining a search
                # to a lookup has to survive it -- this example did not, at
                # first.
